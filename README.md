...
### MacBook 내장 웹캠 사용하기 (macOS 전용)
Docker for Mac에서는 컨테이너가 내장 웹캠에 직접 접근할 수 없습니다. 따라서 호스트(Mac)에서 영상을 스트리밍하고 Docker가 이를 수신하는 방식을 사용합니다.

1.  **FFmpeg 및 MediaMTX 설치**
    ```bash
    brew install ffmpeg mediamtx
    ```

2.  **MediaMTX 실행** (RTSP 서버)
...
3.  **웹캠 스트리밍 시작**
    터미널 2에서 프로젝트 루트 폴더 내 스크립트를 실행합니다.
    ```bash
...
4.  **VLC로 스트림 확인 (선택 사항)**
    VLC에서 `rtsp://localhost:8554/webcam` 주소를 열어 영상이 잘 나오는지 확인합니다.

5.  **Docker 시스템 실행**
    ```bash
    docker-compose up --build
    ```
    `ai-worker` 로그에서 `카메라 연결 성공: rtsp://host.docker.internal:8554/webcam` 메시지가 뜨면 성공입니다.
